...
void init_reflective_sphere() {
	...
	obj.material = MaterialType::NO_MATERIAL;
	obj.shading = ShadingType::REFLECTION; //  TODO REFLECTION;
...
void init_refractive_obj() {
	...
	obj.diffuseTexID = cubeTexture;
	obj.material = MaterialType::NO_MATERIAL;
	obj.shading = ShadingType::REFRACTION;  // TODO REFRACTION
	obj.name = "Glass Cube";
...
void init_textured_plane() {
	...
	/*std::vector<glm::vec3> color = { { 230, 68, 32 } , { 230, 124, 32 } , { 230, 187, 32 } , { 230, 220, 32 } };
	...
	for (c = 0; c < 4; c++) {
		for (i = 0; i < 16; i++) {
			for (j = 0; j < 64; j++) {
				image[i][j][0] = (GLubyte)color[c].r;
				image[i][j][1] = (GLubyte)color[c].g;
				image[i][j][2] = (GLubyte)color[c].b;
			}
		}
	}*/
	/////////////////////////////////////////
...
void init_skybox() {
	Mesh surface = {};
	loadObjFile(MeshDir + "cube_n_t_flat.obj", &surface);
	// reverse draw order to counter ClockWise face culling
	//così vedo le facce e la texture "da dentro"
	std::reverse(surface.vertices.begin(), surface.vertices.end()); 
	generate_and_load_buffers(true, &surface);
	Object obj = {};
	obj.mesh = surface;
	obj.diffuseTexID = cubeTexture;
	...
	obj.name = "skybox";
	//Posizionamento cubo contenente la scena (il cubo è già statto creato nell'orgine, quindi non devo traslarlo)
...
void init_torus() {
	...
	obj.shading = ShadingType::TEXTURE_PHONG;  //TEXTURE_ONLY; // PASS_THROUGH; 
	obj.name = "Torus";

	/************************************************************************************
	*									Image Texture									*
	************************************************************************************/
	//load texture image and bring it into memory (GPU)
	...
	* Compute checkboard procedural_texture image of dimension width x width x 3 (RGB)	*
	************************************************************************************/
	//divinding texture image in four columns with four different colors
	std::vector<glm::vec3> color = { { 230, 68, 32 } , { 230, 124, 32 } , { 230, 187, 32 } , { 230, 220, 32 } };
	...
	for (c = 0; c < 4; c++) {
		for (i = 0; i < 64; i++) {
			for (j = 0; j < 16; j++) {
				image[i][j + c * 16][0] = (GLubyte)color[c].r;
				image[i][j + c * 16][1] = (GLubyte)color[c].g;
				image[i][j + c * 16][2] = (GLubyte)color[c].b;
			}
		}
	}
	/////////////////////////////////////////
...
void initShader()
{
	...
	glUniform1i(glGetUniformLocation(shaders_IDs[SKYBOX], "cubemap"), 0);
	...
	base_unif.camera_position_pointer = glGetUniformLocation(shaders_IDs[REFLECTION], "camera_position");
	//Rendiamo attivo lo shader
	glUseProgram(shaders_IDs[REFLECTION]);
	glUniform1i(glGetUniformLocation(shaders_IDs[REFLECTION], "cubemap"), 0);
	...
	base_unif.camera_position_pointer = glGetUniformLocation(shaders_IDs[REFRACTION], "camera_position");
	//Rendiamo attivo lo shader
	glUseProgram(shaders_IDs[REFRACTION]);
	glUniform1i(glGetUniformLocation(shaders_IDs[REFRACTION], "cubemap"), 0);
...
void computeTorusVertex(int i, int j, Mesh* mesh) {
	...
	//aggiungo anche la texture e quindi i valori s e t per quel vertice
	//per esempio, potremmo assegnare phi e theta
	//dato che s,t -> [0,1] , possiamo dividere per pig
...
//stdi_load si occupa di caricare da qualsiasi formato solo le informazioni utili
//(altezza, karghezza e canali dell'immagine
GLuint loadTexture(string path)
{
	...
	// data loading in memory (GPU) from CPU!
	glTexImage2D(GL_TEXTURE_2D,  //the target
		0, // the mip map level we want to generate
		format,
		width,
