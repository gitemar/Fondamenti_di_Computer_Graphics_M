...
void init_axis() {
	...
	Object obj5 = {};
	obj5.mesh = _grid;
	obj5.material = MaterialType::NO_MATERIAL;
	obj5.shading = ShadingType::BLINN;
	obj5.name = "axis_";
	obj5.M = glm::scale(glm::mat4(1), glm::vec3(2.f, 2.f, 2.f));
	Axis = obj5;
}
...
void initShader()
{
	...
	light_uniforms.resize(6); // allocate space for uniforms of PHONG, BLINN and GOURAUD + TOON, WAVE
	base_uniforms.resize(NUM_SHADERS); // allocate space for uniforms of PHONG,BLINN,GOURAUD,TOON,WAVE
	...
	base_unif.MWCS_Matrix_pointer = glGetUniformLocation(shaders_IDs[GOURAUD], "MWCS");
...
void init() {
	...
	light.initial_pos = light.position;
	...
	materials[MaterialType::ROSE_GOLD].name = "Rose Gold";
	materials[MaterialType::ROSE_GOLD].ambient = rose_gold_ambient;
	materials[MaterialType::ROSE_GOLD].diffuse = rose_gold_diffuse;
	materials[MaterialType::ROSE_GOLD].specular = rose_gold_specular;
	materials[MaterialType::ROSE_GOLD].shininess = rose_gold_shiness;
...
void drawScene() {
	...
	for (int i = 0; i < objects.size(); i++) {
		...
		case ShadingType::WAVE:
			...
			// Time setting
			t = clock();
			glUniform1f(base_uniforms[WAVE].time_delta_pointer, t); 
			//Material loading
		...
		glUniform3f(light_uniforms[objects[i].shading].light_position_pointer, light.position.x, light.position.y, light.position.z);
		glUniform3f(light_uniforms[objects[i].shading].light_color_pointer, light.color.r, light.color.g, light.color.b);
		glUniform1f(light_uniforms[objects[i].shading].light_power_pointer, light.power);
...
void material_menu_function(int option)
{
	/*cout << "Name: " << objects[selected_obj].name << endl;
	cout << "Material: " << (MaterialType)option << endl;*/
...
void buildOpenGLMenu()
{
	...
	glutAddMenuEntry(materials[MaterialType::ROSE_GOLD].name.c_str(), MaterialType::ROSE_GOLD);
...
void loadObjFile(string file_path, Mesh* mesh)
{
	...
	// Obbligatorio perché la scena tridimensionale deve gestire le luci -> DOBBIAMO conoscere la normale ai vertici o alle facce
	if (tmp_normals.size() == 0) {
		...
		for (int i = 0; i < vertexIndices.size(); i += 3)
		{
			...
			//Calcoliamo la normale al vertice, non alla faccia
			//su ogni vertice c'è la somma delle normali di tutti i triangoli che incidono su quel vertice
			//le normali non sono più "normalizzate", quindi prima di passare tmp_normals dobbiamo normalizzarlo
		}
	}
